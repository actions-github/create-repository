...
(async function () {
    ...
    try {
        console.log(`Checking if repository ${targetRepoName} is already exists...`);
        ...
        if (checkRepoExists.statusCode === 200) {
            ...
            console.log(`Creating repository ${targetRepoName} ${isPrivate == "true" ? "(private)" : "(public)"} ...`);
            ...
            if (createRepoResponse.statusCode === 201) {
                console.log(`Repository ${targetRepoName} ${isPrivate == "true" ? "(private)" : "(public)"} created successfully!`);
                core.setOutput("repo-url", "https://github.com/" + targetOrgName + "/" + targetRepoName);
            }
            else {
                core.setOutput("repo-url", "");
                core.setFailed(`Failed to create repository: ${createRepoResponse.statusCode}`);
            }
        }
        else {
            core.setOutput("repo-url", "");
            core.setFailed(`Failed to check repository: ${checkRepoExists.statusCode}`);
        }
    }
    catch (error) {
        // failed
